...
def test_clinvar_get_latest_version(clinvar_source_config):
    ...
    with patch("opendatalake.lib.domain.source_configs.clinvar.http_get", return_value=mock_response) as mock_http_get:
...
def test_clinvar_get_latest_version_no_match_raises(clinvar_source_config):
    ...
    with (
        patch("opendatalake.lib.domain.source_configs.clinvar.http_get", return_value=mock_response),
