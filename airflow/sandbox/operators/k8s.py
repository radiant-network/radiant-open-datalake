...
class PythonScriptOperator(KubernetesPodOperator):
    def __init__(self, script_name, script_args, **kwargs):
        ...
        # ECS-only kwargs (secrets injected via Secrets Manager -> ECS task def). The sandbox pod gets
        # its secrets from cluster-wide env vars (airflow-values.yaml extraEnv), so drop them here
        # rather than forwarding to KubernetesPodOperator.
