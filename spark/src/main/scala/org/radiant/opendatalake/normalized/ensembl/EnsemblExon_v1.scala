...
/** One row per (exon, transcript) of the GFF3 release, carrying the transcript gene and flags. */
case class EnsemblExon_v1(rc: RuntimeETLContext, version: String, rawStorage: String, tablePrefix: String, database: Option[String] = None, override val warehouse: Option[String] = None)
  extends ContractETLP(rc, sourceDatasetId = "normalized_ensembl_exon", tablePrefix, major = 1, database) {
  ...
  override def transformSingle(data: Map[String, DataFrame],
    ...
    EnsemblGff.exons(EnsemblGff.base(data(raw_ensembl_gff3.id)))
